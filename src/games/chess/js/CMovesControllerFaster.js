...
function CMovesControllerFaster(aMatrix){
    ...
    this.getKingMove = function(iRow, iCol, aBoard){
        ...
        if(aKingHistory.length===1 && !s_oGame.isCheck()){
            ...
            if(bCastlingLeft){
                aNeighborList.push({row: iRow, col:2, special: BOARD_SPECIAL_CASTLING_LEFT});
            }
        }
    ...
    this._getEnPassant = function(szColor, iRow, iCol, aBoard){
        ...
        ////ADD SPECIAL TAG, TO AVOID COMPUTATION FOR CPU
        for(var i=0; i<aList.length; i++){
            aList[i].special = BOARD_SPECIAL_ENPASSANT;
        }
        ...
        for(var i=0; i<aCurPlayerPieces.length; i++){
            ...
            for(var j=0; j<aCurPieceMoveList.length; j++){
                ...
                var oMove = {   sourcerow:iSourceRow, sourcecol:iSourceCol, destrow:iDestRow, destcol:iDestCol, 
                                        ...
                                        special:aCurPieceMoveList[j].special
                };
    ...
    this._makeMove = function(aBoard, oMove){    
        ...
        switch(oMove.special){
            case BOARD_SPECIAL_CASTLING_RIGHT:{
                    aBoard[oMove.sourcerow][5] = {color: oMove.sourcecolor, type: ROOK, history: aBoard[oMove.sourcerow][7].history};
                    aBoard[oMove.sourcerow][7] = {color: null, type: null, history: []};
                    break;
            }
            case BOARD_SPECIAL_CASTLING_LEFT:{
                    aBoard[oMove.sourcerow][3] = {color: oMove.sourcecolor, type: ROOK, history: aBoard[oMove.sourcerow][0].history};
                    aBoard[oMove.sourcerow][0] = {color: null, type: null, history: []};
                    break;
            }
            case BOARD_SPECIAL_ENPASSANT:{
                    var szOpponentColor = s_oBoardStateController.getOtherOpponent(oMove.sourcecolor);
                    aBoard[oMove.sourcerow][oMove.destcol] = {color: null, type: null, history: []};
                    break;
            }
        }
    ...
    this._unMakeMove = function(aBoard, oMove){     
        ...
        switch(oMove.special){
            case BOARD_SPECIAL_CASTLING_RIGHT:{
                    aBoard[oMove.sourcerow][7] = {color: oMove.sourcecolor, type: ROOK, history: aBoard[oMove.sourcerow][5].history};
                    aBoard[oMove.sourcerow][5] = {color: null, type: null, history: []};
                    break;
            }
            case BOARD_SPECIAL_CASTLING_LEFT:{
                    aBoard[oMove.sourcerow][0] = {color: oMove.sourcecolor, type: ROOK, history: aBoard[oMove.sourcerow][3].history};
                    aBoard[oMove.sourcerow][3] = {color: null, type: null, history: []};
                    break;
            }
            case BOARD_SPECIAL_ENPASSANT:{
                    var szOpponentColor = s_oBoardStateController.getOtherOpponent(oMove.sourcecolor);
                    aBoard[oMove.sourcerow][oMove.destcol] = {color: szOpponentColor, type: PAWN, history: []};
                    break;
            }
        }
