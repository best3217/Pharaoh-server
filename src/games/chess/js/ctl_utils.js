...
function smartResize(oElement, iXTolerance, iYTolerance){
    ...
    var iWidthToFit = CANVAS_WIDTH- (2*s_iOffsetX) - iXTolerance;
    var iHeightToFit = CANVAS_HEIGHT- (2*s_iOffsetY) - iYTolerance;
    
    var iLowerSize = inRectResize(oElement, iWidthToFit, iHeightToFit);
   
    oElement.scaleX = oElement.scaleY = 1;
    if(iLowerSize < 1){
        oElement.scaleX = oElement.scaleY = iLowerSize;
    }
    
    return oElement.scaleX;
};

function zoomInCamera(oElement, iCameraWidth, iCameraHeight, iElementWidth, iElementHeight){
    if(oElement.getBounds() === null || oElement.getBounds() === undefined ){
        return;
    }
    
    var iScaleX = iCameraWidth/(iElementWidth);
    var iScaleY = iCameraHeight/(iElementHeight);

    var iZoomToApply = iScaleX <= iScaleY ? iScaleX : iScaleY;

    oElement.scaleX = oElement.scaleY = iZoomToApply;
...
function inRectResize(oElement, iWidth, iHeight){ 
    ...
    var iScaleRatioX = _getNormalizedSize(oElement.getBounds().width, iWidth);
    var iScaleRatioY = _getNormalizedSize(oElement.getBounds().height, iHeight);
    
    var iScaleToApply = Math.min(iScaleRatioX, iScaleRatioY);
    
    oElement.scaleX = oElement.scaleY = iScaleToApply;
