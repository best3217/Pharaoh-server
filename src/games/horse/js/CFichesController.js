function CFichesController(iScale,oParentContainer){
    ...
    this.refreshFiches = function(aFiches,iXPos,iYPos){
        ...
        var iXOffset=iXPos+(FICHE_WIDTH*iScale)/2;
        var iYOffset=iYPos+(FICHE_WIDTH*iScale)/2;
        ...
        for(var i=0;i<aFiches.length;i++){
                var oSpriteChip = new CFicheBut(aFiches[i].index,iXOffset,iYOffset,iScale,_oFichesAttach);//s_oSpriteLibrary.getSprite("fiche_"+aFiches[i].index);
                oSpriteChip.disable();
