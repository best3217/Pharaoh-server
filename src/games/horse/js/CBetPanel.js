function CBetPanel(){
    ...
    this._init = function(){
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            ...
            _pStartPosAudio = {x: CANVAS_WIDTH - oSpriteExit.width/2 - 10, y: (oSprite.height / 2) + 10};
            _oAudioToggle = new CToggle(_pStartPosAudio.x, _pStartPosAudio.y, oSprite, s_bAudioActive,s_oStage);
            _oAudioToggle.addEventListener(ON_MOUSE_UP, this._onAudioToggle, this);
            _pStartPosFullscreen = {x: _pStartPosAudio.x - oSprite.width/2  - 10,y:_pStartPosAudio.y};
        }else{
            _pStartPosFullscreen = {x: CANVAS_WIDTH - oSpriteExit.width/2 - 10, y: oSpriteExit.height/2 + 10};
        }
    ...
    this.setSimpleBet = function(iIndex,iPlace,iFicheValue,oBut){
        if(iFicheValue > s_iCurMoney ){
            ...
            _oMsgBox.show(TEXT_NO_MONEY,true);
        ...
        if( (_iTotBet+iFicheValue) > MAX_BET){
            ...
            _oMsgBox.show(TEXT_ERR_MAX_BET,false);
